...
export default function Dashboard() {
    ...
    const fetchAllMovies = async () => {
      try {
        ...
        endpoints.forEach((ep, index) => {
          // Removes movies that don't have a background poster to avoid blank boxes
          newObj[ep.key] = data[index].results ? data[index].results.filter(m => m.backdrop_path) : [];
        });

        setMovieData(newObj);
        if (newObj.trending.length > 0) {
          setHeroMovie(newObj.trending[Math.floor(Math.random() * newObj.trending.length)]);
        }
      } catch (error) {
      ...
      if (currentUser) {
        ...
        unsubscribeFirestore = onSnapshot(userDocRef, (docSnap) => {
          if (docSnap.exists()) {
            ...
            setUserData({ uid: currentUser.uid, name: data.name || currentUser.displayName || 'User', email: data.email || currentUser.email || '', role: data.role || 'standard_user', plan: data.subscriptionPlan || 'free', photoURL: currentUser.photoURL || '' });
  ...
  const handleAction = async (actionType, e = null) => {
    ...
    if (actionType === 'logout') {
      localStorage.clear(); await signOut(auth); navigate('/login');
    }
  };
  ...
  // 🔥 PERFECT HOVER CARD WITH FIXES 🔥
  const NetflixCard = ({ movie, index, isTop10 }) => {
    ...
    const handleMouseEnter = () => {
      setIsHovered(true); // Instant Expansion
      
      // Video trailer loads after 800ms so scrolling over movies doesn't lag the browser
      hoverTimeoutRef.current = setTimeout(async () => {
        if (!trailerKey) {
          ...
          } catch (e) { console.error("Trailer fetch error"); }
        } else {
          setShowTrailer(true);
        }
      }, 800);
    ...
    // Dynamic Origins to keep cards on screen
    let originClass = "origin-center";
    if (index === 0) originClass = "origin-left";
    else if (index > 4 && index % 5 === 4) originClass = "origin-right";

    return (
      <div className={`netflix-card-container ${originClass}`} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        
        <div className="card-image-wrapper">
          {/* Always show image, overlay trailer on top to prevent black box */}
          <img src={`${TMDB_IMAGE_BASE_URL}${imagePath}`} alt={title} className="netflix-card-img" />
          ...
          {showTrailer && trailerKey && (
            <iframe 
              ...
              className="netflix-card-trailer"
              frameBorder="0"
            />
          )}

          {/* Badges exactly like your screenshot */}
          {isTop10 && <div className="net-top10-badge">TOP<br/>10</div>}
          <div className="recently-added-badge">Recently added</div>
        </div>
        
        {/* Title overlays the image when NOT hovered */}
        <div className="net-movie-title-overlay">{title}</div>

        {/* Hover Info Panel (Exact match to your screenshot) */}
        <div className="netflix-card-details">
          
          {/* Action Buttons */}
          <div className="ncd-buttons">
            <div className="ncd-actions-left">
              <button className="ncd-btn play-btn-exact" onClick={() => showCustomToast(`Playing ${title}`, 'success')} title="Play">
                <svg viewBox="0 0 24 24" width="20" height="20" fill="black"><path d="M6 4l15 8-15 8z"></path></svg>
              </button>
              <button className="ncd-btn round-btn" onClick={() => showCustomToast(`Added to My List`, 'success')} title="Add">
                <svg viewBox="0 0 24 24" width="16" height="16" fill="white"><path d="M11 11V5h2v6h6v2h-6v6h-2v-6H5v-2h6z"></path></svg>
              </button>
              <button className="ncd-btn round-btn" title="Like">
                <svg viewBox="0 0 24 24" width="14" height="14" fill="white"><path d="M1 21h4V9H1v12zm22-11c0-1.1-.9-2-2-2h-6.31l.95-4.57.03-.32c0-.41-.17-.79-.44-1.06L14.17 1 7.59 7.59C7.22 7.95 7 8.45 7 9v10c0 1.1.9 2 2 2h9c.83 0 1.54-.5 1.84-1.22l3.02-7.05c.09-.23.14-.47.14-.73v-2z"></path></svg>
              </button>
            </div>
            <button className="ncd-btn round-btn info-btn" title="More Info">
          ...
          {/* Meta Info (A • 2h 15m • HD) */}
          <div className="ncd-meta">
            <span className="ncd-match">98% Match</span>
            <span className="ncd-age-badge">A</span>
            <span className="dot">•</span>
            <span>{isTV ? '6 Episodes' : '2h 15m'}</span>
            <span className="dot">•</span>
            <span className="ncd-hd-badge">HD</span>
          </div>

          {/* Genres */}
          <div className="ncd-genres">
            Gritty <span className="pipe">•</span> Action <span className="pipe">•</span> Thriller
          </div>
  ...
  const NetflixRow = ({ title, movies, isTop10Row }) => {
    ...
    const scroll = (direction) => {
      if (rowRef.current) {
        ...
        if (direction === 'left' && scrollTo <= 0) setShowLeftArrow(false);
      }
    };
    ...
    return (
      <div className="netflix-row-wrapper">
        ...
        <div className="netflix-row-container">
          ...
          {showLeftArrow && (
            <button className="slider-arrow left-arrow" onClick={() => scroll('left')}>&#10094;</button>
          )}
          
          <div className="netflix-row" ref={rowRef}>
            {movies.map((movie, index) => (
              <NetflixCard key={`${title}-${movie.id}-${index}`} movie={movie} index={index} isTop10={isTop10Row && index < 5} />
            ))}
          </div>
    ...
    <div className="dashboard-container">
      ...
      <nav className={`net-navbar ${isScrolled || activeTab !== 'home' ? 'scrolled' : ''}`}>
        ...
        <div className="net-nav-right">
          ...
          <div className="net-profile-trigger" onClick={() => handleAction('logout')}>
          ...
          <div className="tab-content animate-fade-in">
            ...
            {searchQuery.trim() !== '' ? (
              ...
              <>
                ...
                {/* 10+ NETFLIX CATEGORIES */}
                <div className="net-sliders-container">
                  <NetflixRow title={currentProfile === 'Kids' ? 'Animation & Fun' : 'Trending Now'} movies={currentProfile === 'Kids' ? movieData.kids : movieData.trending} isTop10Row={true} />
                  <NetflixRow title={currentProfile === 'Kids' ? 'Learn & Play' : 'Top Rated Masterpieces'} movies={currentProfile === 'Kids' ? movieData.kids.slice().reverse() : movieData.topRated} />
                  <NetflixRow title={currentProfile === 'Kids' ? 'Family Movie Night' : 'Action & Adventure'} movies={currentProfile === 'Kids' ? movieData.kids : movieData.action} />
                  
                  {currentProfile !== 'Kids' && (
                    <>
                      <NetflixRow title="Sci-Fi & Fantasy" movies={movieData.sciFi} />
                      <NetflixRow title="Laugh Out Loud" movies={movieData.comedy} />
                      <NetflixRow title="Gritty & Ominous Thrillers" movies={movieData.thriller} />
                      <NetflixRow title="Crime Detectives" movies={movieData.crime} />
                      <NetflixRow title="Epic Romances" movies={movieData.romance} />
                      <NetflixRow title="Chills & Horror" movies={movieData.horror} />
                      <NetflixRow title="Mysteries" movies={movieData.mystery} />
                    </>
                  )}
                </div>
              </>
...
const InlineStyles = () => (
  <style>{`
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .brand-text-colored { font-size: 22px; font-weight: 900; color: #e50914; letter-spacing: 1px; }
    ...
    /* 🔥 HERO BANNER - TEXT OVERLAP FIXED 🔥 */
    ...
    /* =========================================================
       🔥 SLIDER AND HOVER CARDS (CUT FIX & INSTANT HOVER) 🔥
       ========================================================= */
    ...
    /* SLIDER ARROWS */
    .slider-arrow { position: absolute; top: 50px; bottom: 140px; width: 4%; background: rgba(0,0,0,0.5); border: none; color: white; font-size: 40px; cursor: pointer; z-index: 20; opacity: 0; transition: opacity 0.3s ease, background 0.3s ease; display: flex; align-items: center; justify-content: center; }
    ...
    /* 🔥 PADDING TRICK TO FIX CUT & SCROLL LOCK 🔥 */
    .netflix-row { 
      display: flex; gap: 8px; 
      overflow-x: scroll; /* Allows JS to scroll without mouse issues */
      overflow-y: hidden;
      padding-top: 150px; padding-bottom: 250px; /* BIG PADDING PREVENTS CUTOFF */
    ...
    /* THE CARD ITSELF */
    .netflix-card-container { min-width: 260px; width: 260px; height: 146px; background-color: #141414; border-radius: 4px; position: relative; cursor: pointer; transition: transform 0.2s ease, box-shadow 0.2s ease; z-index: 1; }
    ...
    .net-movie-title-overlay { position: absolute; bottom: 5px; left: 10px; right: 10px; font-weight: bold; font-size: 14px; text-shadow: 1px 1px 3px black; z-index: 5; text-align: center; opacity: 1; transition: 0.2s;}
    ...
    /* RECENTLY ADDED BADGE POSITIONED LIKE SCREENSHOT */
    .recently-added-badge { position: absolute; bottom: 5px; left: 50%; transform: translateX(-50%); background: #e50914; color: white; padding: 4px 10px; font-size: 11px; font-weight: bold; border-radius: 4px; z-index: 10; opacity: 0; transition: opacity 0.2s; white-space: nowrap; box-shadow: 0 2px 5px rgba(0,0,0,0.5); }

    /* HOVER POPUP DETAILS (The dark panel) */
    .netflix-card-details { position: absolute; top: 100%; left: 0; right: 0; background-color: #181818; padding: 15px; border-bottom-left-radius: 6px; border-bottom-right-radius: 6px; opacity: 0; visibility: hidden; transition: opacity 0.2s ease, visibility 0.2s ease; box-shadow: 0 15px 25px rgba(0,0,0,1); z-index: 100; border: 1px solid #333; border-top: none; display: flex; flex-direction: column; gap: 12px; }
    
    /* 🔥 INSTANT ON HOVER EFFECT (REMOVED DELAY) 🔥 */
    .netflix-card-container:hover { transform: scale(1.4) !important; z-index: 1000; box-shadow: 0 15px 40px rgba(0,0,0,1); border-bottom-left-radius: 0; border-bottom-right-radius: 0; transition-delay: 0s;}
    ...
    .ncd-btn { width: 38px; height: 38px; border-radius: 50%; border: 2px solid rgba(255,255,255,0.4); background: #2a2a2a; color: white; display: flex; justify-content: center; align-items: center; cursor: pointer; transition: 0.2s; }
    .ncd-btn:hover { border-color: white; background: #444; }
    .play-btn-exact { background: white; color: black; border-color: white; padding-left: 2px; }
    .play-btn-exact:hover { background: #e5e5e5; }

    /* HOVER CARD: META (Age, Time, HD) */
    ...
    .ncd-age-badge { background: transparent; border: 1px solid #808080; padding: 2px 6px; border-radius: 2px; color: white; font-size: 11px; }
    .ncd-hd-badge { border: 1px solid #808080; padding: 2px 4px; border-radius: 2px; font-size: 10px; color: white; }
    .ncd-meta .dot { font-size: 10px; }

    /* HOVER CARD: GENRES */
    .ncd-genres { font-size: 13px; color: #fff; font-weight: bold; display: flex; align-items: center;}
